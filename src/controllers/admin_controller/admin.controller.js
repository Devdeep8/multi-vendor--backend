...
exports.getDashboardData = async (req, res) => {
  try {
    ...
    const previousPeriodRevenue = await db.OrderItem.sum('price', {
      where: {
        createdAt: {
    ...
    const revenueChange = calculatePercentageChange(totalRevenue, previousPeriodRevenue);
    ...
    const usersChange = calculatePercentageChange(totalUsers, previousPeriodUsers);
    ...
    const ordersChange = calculatePercentageChange(totalOrders, previousPeriodOrders);
    ...
    const productsChange = calculatePercentageChange(totalProducts, previousPeriodProducts);
    
    console.log("dashboard is working finr " ,productsChange,totalProducts, totalRevenue , revenueChange , ordersChange , totalOrders , totalUsers , usersChange )
      ...
      include: [
        ...
        },
        {
          ...
          include: [
            {
              model: db.ProductVariant,
              attributes: ['id', 'size', 'color', 'additional_price', 'image_url'],
              include: [
                {
                  model: db.Product,
  ...
  try {
    const topSellers = await db.Seller.findAll({
      attributes: [
        ...
        [db.sequelize.fn('COUNT', db.sequelize.col('Products.ProductVariants.OrderItems.id')), 'sales'],
        [db.sequelize.fn('SUM', db.sequelize.col('Products.ProductVariants.OrderItems.price')), 'revenue']
      ],
      include: [
        {
          model: db.User,
  ...
  try {
    const sellers = await db.User.findAll({
      attributes: [
        ...
        [db.sequelize.fn('COUNT', db.sequelize.col('Products.id')), 'totalProducts'],
        // Total sold products (sum of quantities or count of order items)
        [db.sequelize.fn('COUNT', db.sequelize.col('OrderItems.id')), 'totalSales'],
        // Total revenue
        [db.sequelize.fn('SUM', db.sequelize.col('OrderItems.price')), 'revenue']
      ],
      include: [
        {
          model: db.Product,
          as: 'Products',
          attributes: [],  // no need for product fields, just count
          required: false,
        },
        {
          model: db.OrderItem,
  ...
  try {
    const seller = await db.User.findOne({
      ...
      include: [
        ...
        {
          model: db.OrderItem,
          as: 'OrderItems', // alias from association
          include: [
            {
              model: db.Order,
              as: 'Order', // alias from association if used
              attributes: ['id', 'payment_status', 'created_at']
            }
          ]
        }
      ]
    ...
    const totalRevenue = seller.OrderItems.reduce((acc, item) => acc + parseFloat(item.price || 0), 0);
    const totalOrders = new Set(seller.OrderItems.map(item => item.Order?.id)).size;
...
exports.getLatestActivities = async (req, res) => {
  try {
    ...
    const activities = [
      ...
      ...recentProducts.map(product => ({
        ...
        user: {
          ...
          avatar: product.Seller?.User?.profileImage || "/images/users/default.png",
          type: "seller",
        },
    ...
    res.status(500).json({ message: 'Error fetching latest activities', error: error.message });
  }
};
...
exports.createProduct = async (req, res) => {
  try {
    const { name, description, price, categoryId, sellerId, stock, status } = req.body;
  ...
  try {
    ...
    if (interval === 'day') {
      ...
      for (let date = moment(start); date <= moment(end); date.add(1, 'day')) {
        ...
        const sales = await db.Order.sum('total_amount', {
          where: {
            created_at: {
      ...
      for (let date = moment(start); date <= moment(end); date.add(1, 'week')) {
        ...
        const sales = await db.Order.sum('total', {
          where: {
            created_at: {
        ...
        salesData.push({
          date: `${date.format('YYYY-MM-DD')} to ${moment(weekEnd).format('YYYY-MM-DD')}`,
          sales,
          orderCount
        });
      }
      ...
      for (let date = moment(start).startOf('month'); date <= moment(end); date.add(1, 'month')) {
...
exports.getUsersReport = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error generating users report', error: error.message });
  }
};
  ...
  try {
    ...
    const topProducts = await db.Product.findAll({
      ...
      include: [
        {
          model: db.OrderItem,
          as: 'OrderItems', // match the association alias
          attributes: []
        },
        {
          model: db.Category,
    ...
    console.log(topProducts , categories, lowStockProducts, outOfStockProducts, "working")
    // Respond with all report data
    res.status(200).json({
      topProducts,
      categories,
      lowStockProducts,
